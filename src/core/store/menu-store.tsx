...
export const useMenuStore = create<MenuStore>()(
  persist(
    (set, get) => ({
      ...
      totalAmount: () =>
        get().cartItems.reduce((sum, i) => sum + i.totalPrice, 0),
      ...
      filteredItems: () => {
        ...
        const findCategoryName = (catId?: string) =>
          categories.find((c) => String(c.id) === String(catId))?.name?.toLowerCase() || "";
        ...
        return menuItems.filter((item) => {
          ...
          const matchesOptions =
            item.options?.some((opt) => {
              ...
              const choiceMatch =
                opt.choices?.some((ch) => ch.name?.toLowerCase().includes(searchTerm)) || false;
          ...
          const matchesCategoryFilter =
            selectedCategory === "All" || String(item.category) === String(selectedCategory);
